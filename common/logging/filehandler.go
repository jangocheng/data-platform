...
func NewTimeRotatingFileHandler(baseName string, when int8, interval int) (*TimeRotatingFileHandler, error) {
	...
	case WhenMinute:
		h.rolloverAt = fInfo.ModTime().Unix() - int64(fInfo.ModTime().Second()) + h.interval
	case WhenHour:
		h.rolloverAt = fInfo.ModTime().Unix() - int64(fInfo.ModTime().Second()) -
			int64(fInfo.ModTime().Minute()) + h.interval
	case WhenDay:
		h.rolloverAt = fInfo.ModTime().Unix() - int64(fInfo.ModTime().Second()) -
			int64(fInfo.ModTime().Minute()) -
			int64(fInfo.ModTime().Hour())  + h.interval
	}
	return h, nil
}

...
func GetMicroWebService(cnf interface{}, opts ...web.Option) (web.Service, error) {
	...
	service := web.NewService(append(opts, web.Flags(flagOptions...))...)
	return service, nil
}

func InitWebService(service web.Service, cnf interface{}, opts ...web.Option) error {
	opts = append(opts, web.Action(func(context *cli.Context) {
		for i := 0; i < reflect.ValueOf(cnf).Elem().NumField(); i++ {

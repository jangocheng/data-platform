...
func (d *DataSourceService) queryDBData(item DatabaseSource, params map[string]interface{}) (string, error) {
	...
	case "oracle":
		oracleClient, ok := d.oraclePool[item.OrgCode]
		if !ok {
			return "", errors.New("can not find sql client by org code")
		}
		instances := make([]map[string]interface{}, 0)
		if item.SqlQueryStr != "" {
			sqlStr := GetSqlStr(item.SqlQueryStr, item.Params, params)
			err := oracleClient.Query(&instances, sqlStr)
			if err != nil {
				return "", err
			}

		} else {
			whereStr, args := GetOracleCondition(item.Params, params)
			...
			for _, tName := range tableList {
				...
				result, err := oracleClient.Find(&condition)
				if err != nil {
					return "", err
				}
				instances = result

...
type DataSourceService struct {
	...
	oraclePool      map[string]*oracle.Client
	killer          *utils.Killer
}
...
func NewDataSourceService(cnf *DataSourceConfig) (*DataSourceService, error) {
	...
	d := &DataSourceService{
		...
		oraclePool:  make(map[string]*oracle.Client),
		killer: killer,
	}
	if cnf.Migrate != "" {
...
func (d *DataSourceService) KeepDBPools(tNames ...string) {
	...
	for {
		...
		for orgCode, orgItem := range dataOwnerMapInstance {
			...
			if orgItem.Engine != "" && orgItem.Db != "" && orgItem.PassWord != "" &&
				...
				case "oracle":
					...
					oracleClient, err := oracle.NewClient(conf)
					if err != nil {
						errStr = err.Error()
						break
					}
					if client, ok := d.oraclePool[orgCode]; ok {
						if client != nil {
							_ = client.Close()
						}
					}
					orgItemHashMap[orgCode] = itemHash
					d.oraclePool[orgCode] = oracleClient
				case "redis":
					jsonBytes, _ := json.Marshal(orgItem)
					itemHash := utils.GetMd5(string(jsonBytes))
		...
		for orgCode, client := range d.oraclePool {
			if _, ok := dataOwnerMapInstance[orgCode]; !ok {
				_ = client.Close()
				delete(d.oraclePool, orgCode)
				delete(orgItemHashMap, orgCode)
			}
		}
...
func (d *DataSourceService) PP() {
	for {
		...
		fmt.Println("oracle_pool", d.oraclePool)
...
func (d *DataSourceService) Close(tNames ...string) error {
	...
	if d.oraclePool != nil {
		for _, sqlClient := range d.oraclePool {
			errSql := sqlClient.Close()
			if errSql != nil {
				errStr = errStr + errSql.Error()
			}
